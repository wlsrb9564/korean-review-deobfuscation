...
| 프롬프트 포맷 | 난독화 복원 전용 포맷 (`### Instruction` / `### Response`) |
| Loss 마스킹 | `DataCollatorForCompletionOnlyLM` — Response 이후만 학습 |
| Attention | Flash Attention 2 |

학습 완료 후 LoRA 어댑터를 base 모델에 병합(`merge_and_unload`)한 뒤 full 모델을 HuggingFace에 업로드하여 vLLM 추론에 사용했습니다.
...
| 학습 steps | 500 steps |
| 선택 체크포인트 | `checkpoint-200` — validation loss 기준 최적 성능 |

학습 완료 후 validation loss 기준으로 `checkpoint-200`을 최적 체크포인트로 선택했습니다. 이후 LoRA 어댑터를 SFT merged 모델에 병합(`merge_and_unload`)한 뒤 full 모델을 HuggingFace에 업로드하여 vLLM 추론에 사용했습니다.
...
├── sft_finetuning.ipynb      # Step 2: SFT 학습 + 모델 병합 + vLLM 추론
├── dpo_preprocessing.ipynb   # Step 3: DPO 데이터셋 구축
├── dpo_finetuning.ipynb      # Step 4: DPO 학습 + 모델 병합 + vLLM 추론
├── requirements.txt
└── README.md
```
